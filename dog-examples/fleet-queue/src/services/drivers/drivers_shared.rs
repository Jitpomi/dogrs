...
pub fn register_hooks(app: &dog_core::DogApp<serde_json::Value, FleetParams>) -> anyhow::Result<()> {
    app.service("drivers")?.hooks(|h| {
        h.before_find(Arc::new(super::drivers_hooks::BeforeRead));
        h.after_find(Arc::new(super::drivers_hooks::AfterRead));
        h.before_create(Arc::new(super::drivers_hooks::BeforeWrite));
        h.after_create(Arc::new(super::drivers_hooks::AfterWrite));
    });
