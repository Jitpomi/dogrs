...
pub fn register_hooks(app: &dog_core::DogApp<serde_json::Value, FleetParams>) -> anyhow::Result<()> {
    app.service("rules")?.hooks(|h| {
        h.before_find(Arc::new(super::rules_hooks::BeforeRead));
        h.after_find(Arc::new(super::rules_hooks::AfterRead));
        h.before_create(Arc::new(super::rules_hooks::BeforeWrite));
        h.after_create(Arc::new(super::rules_hooks::AfterWrite));
    });
